...
def main():
    ...
    i=0
    while i < len(leg[dof]['dof_pos_target']):
        ...
        if (time.time()-startTime) >= STEP_VALUE:
            ...
            for dof in dofs:
                status = joints[dof].send_safe_rad_command((leg[dof]['dof_pos_target'][i]), SPEED_VALUE, KP_VALUE, KD_VALUE, TORQUE_VALUE)
                if status != None:
                    pos, vel, curr =  joints[dof].motor_status.act_pos, joints[dof].motor_status.act_vel,joints[dof].motor_status.act_torque
